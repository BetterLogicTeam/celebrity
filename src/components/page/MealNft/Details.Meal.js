...
function MealDetails({ expiryTimestamp }) {
  ...
  useEffect(() => {
    ...
    console.log('what is dataUrlNft', dataUrlNft)
  ...
  useEffect(() => {
    axios.get('https://backendpub.celebrity.sg/api/v1/user/all')
      .then(res => {
        console.log('what is reponse', res.data[0].myReferralCode)
            ...
            <div className="d-flex rpv_center" style={{ alignItems: 'flex-end', justifyContent: 'start' }}>
              {
                (!user.walletAddress || user.walletAddress === "undefined") ?
                  ...
                  <Link to="#" className=" justify_content_center mt-4 mb-1">
                    {token === "bnb" &&
                      <button disabled={availableNft < 1} className="card_button button_dtl" onClick={() => {
                        mintCelebrityNft(bnbTwoDec, "0x0000000000000000000000000000000000000000", affiliateWalletAddress, mealnId)
                        mintTicketNFTTestnetBNB({ bnbamount: bnbTwoDec, tokenAddress: '0x0000000000000000000000000000000000000000', nftid: mealnId, price: bnbTwoDec01, nonce: uuidv4(), signature: Date(), refadress: allUsers[0].myReferralCode })
                      }
                      } href="#!">{availableNft < 1 ? "No Nft available" : `BUY THIS NFT FOR ${bnbTwoDec} BNB`}</button>}
                    {token === "usdsc" &&
